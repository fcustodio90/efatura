...
module Efatura
  ...
  def self.invoices(from_date, to_date)
    Client.new(from_date, to_date).invoices
  end
  ...
  class Client
    ...
    def date_valid?(to_date, from_date)
      # FOR A DATE TO BE VALID IT NEEDS TO PASS THE THREE CONDITIONS
      date_format_valid?(from_date) && date_format_valid?(to_date) && date_same_year?
    end
    ...
    def login
      # IF DATE IS VALID EXECUTES THE LOGIN METHOD
        ...
        agent.get(LOGIN_URL) do |login_page|
          ...
          return cookies = Hash[agent.cookie_jar.store.map { |i| i.cookie_value.split('=') }]
        end
      #end
    end
  end
